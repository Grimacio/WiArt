...
def Kalman(array, Q, R):
    ...
    global lastXhat, lastXhatminus, lastP, lastPminus, lastK, gravityVector, lastOutput, lastVelocity
    xhat=[[0],[0],[0]]
    xhatminus=[[0],[0],[0]]
    P=[[0],[0],[0]]
    Pminus=[[0],[0],[0]]
    ...
    for k in range(1,len(array[0])):
        ...
        gravityVector, reset=gravCorrection([xhat[0][k]]+[xhat[1][k]]+[xhat[2][k]], 0.05, gravityVector, correction_cooldown)
        for i in range(3):
            output[i][k]= xhat[i][k]-gravityVector[i]
...
def isEndPoint(accelVector, margin):
    #print(np.linalg.norm(accelVector))
    return np.linalg.norm(accelVector) < 1 +margin/2 and np.linalg.norm(accelVector) > 1 - margin/2

def gravCorrection(accelVector, margin, actual, event):
    global endPoint
    if event.is_set():
        if isEndPoint(accelVector,margin):
            if not endPoint:
                event.clear()
                run_scheduled_task(2, event)
                endPoint= not endPoint
                #print("Endpoint Detected!")
            return (accelVector, True)
        else:
            if endPoint:
                endPoint= not endPoint
                #print("Moving!")
            ...
            while not stop_event.is_set() :
                ...
                for frame in frames:
                    ...
                    if not stop_calibration.is_set():
                        #C_max= max([C_max]+[x]+[y]+[z])
                        #C_min= min([C_min]+[x]+[y]+[z])
                ...
                filteredAcc=Kalman([rawX]+[rawY]+[rawZ], 1e-4, 0.3)
                X_plotData=np.append(X_plotData[len(filteredAcc[0]):], filteredAcc[0])
                Y_plotData=np.append(Y_plotData[len(filteredAcc[1]):], filteredAcc[1])
                Z_plotData=np.append(Z_plotData[len(filteredAcc[2]):], filteredAcc[2])
